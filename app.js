...
const generateElements = (elem, data, dataTextName, type, selectData) => {
    ...
    switch (nodeName) {
        ...
        case elementTypes.select:
            ...
            generateSelect(element, selectData.data, selectData.text, selectData.value);
            break;
...
const tryConvertModelForNewRow = () => {
    ...
    let nationalityName = ddlNationality.options[ddlNationality.selectedIndex].text;
    let positionName = ddlPosition.options[ddlPosition.selectedIndex].text;

    if (ddlGender.value == -1 || ddlNationality.value == -1 || ddlPosition.value == -1) {
...
const saveNewRow = () => {
    ...
    //todo
    // write save new Row methods here
...
const revertDeletedRow = (e) =>{
    let tr = e.target.closest('tr');
    tr.classList.remove('deletedRow');

    tr.cells[0].children[2].remove();
...
const deleteRow = (e) => {
   ...
   let revertIcon = generateIcons('fa-solid fa-undo',revertDeletedRow);
   tr.cells[0].appendChild(revertIcon);
...
const generateTr = (data) => {
    ...
    //icon section
    cells.tdSave.appendChild(generateIcons('fa-solid fa-save', saveEditedRow, true));
    cells.tdSave.appendChild(generateIcons('fa-solid fa-edit', showEditRow, false));

    cells.tdRemove.appendChild(generateIcons('fa-solid fa-undo', undoRow, true));
    cells.tdRemove.appendChild(generateIcons('fa-solid fa-trash-alt', deleteRow, false));

    //id section
    cells.tdId.appendChild(generateElements(elementTypes.span, data, 'id', 'text', null));
    cells.tdId.appendChild(generateElements(elementTypes.input, data, null, 'text', null));

    //firstName section
    cells.tdFirstName.appendChild(generateElements(elementTypes.span, data, 'firstName', 'text', null));
    cells.tdFirstName.appendChild(generateElements(elementTypes.input, data, null, 'text', null));

    //lastName section
    cells.tdLastName.appendChild(generateElements(elementTypes.span, data, 'lastName', 'text', null));
    cells.tdLastName.appendChild(generateElements(elementTypes.input, data, null, 'text', null));

    //patronymic section
    cells.tdPatronymic.appendChild(generateElements(elementTypes.span, data, 'patronymic', 'text', null));
    cells.tdPatronymic.appendChild(generateElements(elementTypes.input, data, null, 'text', null));

    //gender section


    cells.tdGender.appendChild(generateElements(elementTypes.span, data, 'gender', '', null));
    cells.tdGender.appendChild(generateElements(elementTypes.select, data, '', '', {
        data: fakeData[0].genderData,
        text: 'name',
        value: 'name'
    }));
    cells.tdGender.appendChild(generateElements(elementTypes.input, data, 'gender', 'hidden', null));


    //nationality section


    cells.tdNationality.appendChild(generateElements(elementTypes.span, data, 'nationality', '', null));
    cells.tdNationality.appendChild(generateElements(elementTypes.select, data, '', '', {
        data: fakeData[0].nationalityData,
        text: 'name',
        value: 'name'
    }));
    cells.tdNationality.appendChild(generateElements(elementTypes.input, data, 'nationality', 'hidden', null));

    //position section


    cells.tdPosition.appendChild(generateElements(elementTypes.span, data, 'position', '', null));
    cells.tdPosition.appendChild(generateElements(elementTypes.select, data, '', '', {
        data: fakeData[0].positionData,
        text: 'name',
        value: 'name'
    }));
    cells.tdPosition.appendChild(generateElements(elementTypes.input, data, 'position', 'hidden', null));


    //salary section
    cells.tdSalary.appendChild(generateElements(elementTypes.span, data, 'salary', 'text', null));
    cells.tdSalary.appendChild(generateElements(elementTypes.input, data, null, 'text', null));
...
const onFlySearch = (e) =>{
    ...
    let data = fakeData[0].mainData.filter(x=>x.firstName.includes(searchValue));
    if(data.length === 0){
